...
function JSDragDropTree()
{
	var idOfTree;
	var imageFolder;
	var plusIconClassName;
	var minusIcoClassName;
	var maximumDepth;
	var dragNode_source;
	var dragNode_parent;
...
JSDragDropTree.prototype = {
	...
	showHideNode : function(e,inputId){
		...
		if(thisNode.className == JSTreeObj.plusIconClassName){
			thisNode.className = JSTreeObj.minusIconClassName;
			var ul = parentNode.getElementsByTagName('UL')[0];
			ul.style.display='block';
			if(!initExpandedNodes)initExpandedNodes = ',';
			if(initExpandedNodes.indexOf(',' + inputId + ',')<0) initExpandedNodes = initExpandedNodes + inputId + ',';
		}else{
			thisNode.className = JSTreeObj.plusIconClassName;
			parentNode.getElementsByTagName('UL')[0].style.display='none';
			initExpandedNodes = initExpandedNodes.replace(',' + inputId,'');
		}	
		JSTreeObj.Set_Cookie('filehierarchy_expandedNodes',initExpandedNodes,500);			
		return false;						
	},
	// Initialize drag 
	initDrag : function(e){
	...
	dropDragableNodes:function() {
		...
		if(JSTreeObj.dragNode_destination){			
			if(JSTreeObj.insertAsSub){
				...
				var ico = JSTreeObj.dragNode_destination.getElementsByTagName('I')[0];					
				ico.style.visibility='visible';
				ico.className = JSTreeObj.minusIconClassName;
			}else{
			...
			if(lis.length==0){
				...
				ico.style.visibility='hidden';	// Hide [+],[-] icon
	...
	__deleteItem_step2 : function(obj)
	{
		...
		JSTreeObj.__deleteComplete(ajaxIndex,obj);	// Specify function that will be executed after file has been found		
	},
	...
	__saveTextBoxChanges : function(e,inputObj)
	{
		...
		if(inputObj.value.length>0){
			...
			JSTreeObj.__renameComplete(ajaxIndex);	// Specify function that will be executed after file has been found
		}
	},
	...
	initTree : function()
	{
		...
		var menuItems = filehierarchy_tree.getElementsByTagName('LI');	// Get an array of all menu items
		for(var no=0;no<menuItems.length;no++){
			// No children var set ?
			...
			ico.onclick = JSTreeObj.showHideNode;
			...
			if(!noDrag)folderIco.onmousedown = JSTreeObj.initDrag;
			folderIco.onmousemove = JSTreeObj.moveDragableNodes;
		...
		initExpandedNodes = this.Get_Cookie('filehierarchy_expandedNodes');
