document.addEventListener('DOMContentLoaded', initConfigResult);


//-[GENERATED CONFIGURATION]------------------------------------------------------------

function initConfigResult(): void {
    ...
    resultContainer.value = getDefaultResult();
}

function getDefaultResult(): string{
...
function saveResultToClipboard(){
    const resultContainer: HTMLTextAreaElement = document.querySelector(".overview_result textarea.boxContainer") as HTMLTextAreaElement;
    navigator.clipboard.writeText("qf.mass = "+resultContainer.value)
    .then(() => {
        if(resultContainer.value == getDefaultResult()){
            alert("The default configuration was copied to the clipboard without modification.");
        }else{
            alert(`The generated Configuration was copied to clipboard.`);
        }
    })
    .catch((error) => {
...
//-[AUTOCONFIGURATOR]------------------------------------------------------------
function resetAutoConfigurator(){
...
//-[APP]------------------------------------------------------------
function resetApp(){
    resetProjectStructur();
    resetAutoConfigurator();
    initConfigResult();
}
...
window.addEventListener("DOMContentLoaded", (event) => {
    ...
    document.querySelector(".overview_result .tab_head_options .uil-copy").addEventListener('click', saveResultToClipboard);
